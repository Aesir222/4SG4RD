...
namespace Asgard
{
    public partial class __Frigg : Form, IForm
    {
        ...
        private const string initialUrl = "https://www.swansonvitamins.com/price/0.00-9.99";//

        #region LastInitUrl
        private static readonly string[] lastInitialUrl = new[] {
            "/q?page="+ new Random().Next(1,99),
            "/q?sort=brand&page="+ new Random().Next(1,99),
            "/q?sort=price&page="+ new Random().Next(1,99),
            "/q?sort=price_r&page="+ new Random().Next(1,99),
            "/q?sort=toprated&page="+ new Random().Next(1,99)
        };
        #endregion//#signupInput > a //#vanillaBox1 > div > button > div.close-x
        ...
        private async Task<bool> StartFrigg()
        {
            try
            {
                ...
                await browser.ExecuteScript("document.querySelector('div.close-x').click()");
            }
            catch (Exception) { }
            //await Task.Delay(10000);
            //await browser.GetSource("registry");
            bool buttonAddToCart = await browser.ElementVisible("#SortBy", "ExceptionLoad");
            if (buttonAddToCart)
            {
        ...
        private async Task<bool> AddToCart()
        {
            try
            {
               //browser.Screenshot("1.Load");
                ...
                await browser.DisableAlerts();
                string clickAddToCart = @"let allProducts = document.querySelectorAll('button.add-to-cart-button');
                    let randProduct = allProducts[Math.floor(Math.random() * (allProducts.length - 1)) + 1];
                    randProduct.click();";
                await browser.ExecuteScript(clickAddToCart);
            }
            catch (Exception) { }

            bool divNotification = await browser.ElementVisible("div.Notification.Notification--pull-up.Notification--success", "ExceptionAddToCart");
            if (divNotification)
            {
                return await GoToCart();
            }
        ...
        private async Task<bool> GoToCart()
        {
            try
            {
               //browser.Screenshot("2.AddToCart");
                await ConsoleProgressGeneral("Resolviendo teorema de Gibbs.", 30);
                await browser.DisableAlerts();
                string clickGoToCart = "document.querySelector('li.info-bar__li.cart-menu').click()";
                await browser.ExecuteScript(clickGoToCart);

            }
            catch (Exception) { }

            bool anchorCheckout = await browser.ElementVisible("a.button-orange.checkout-btn", "ExceptionGoToCart");
            if (anchorCheckout)
            {
        ...
        private async Task<bool> GoToCheckout()
        {
            try
            {
               //browser.Screenshot("3.GoToCart");
                await ConsoleProgressGeneral("Resolviendo teorema de Gibbs.", 30);
                string clickGoToCheckout = "document.querySelector('a.button-orange.checkout-btn').click()";
                await browser.ExecuteScript(clickGoToCheckout);

            }
            catch (Exception) { }

            bool inputFirstName = await browser.ElementVisible("#firstName", "ExceptionGoToCheckout");
            if (inputFirstName)
            {
        ...
        private async Task<bool> Checkout()
        {
            try
            {
               //browser.Screenshot("4.GoToCheckout");
                //browser.Screenshot("2.LogIn");
                await ConsoleProgressGeneral("Resolviendo teorema de Gibbs.", 30);
                //string selectRandomState = @"let selectState = document.querySelector('#state');";
                //await browser.ExecuteScript(selectRandomState);
                await VerifyVisibleState();

                await browser.JsSendKeys("#firstName", "");
                await browser.JsSendKeys("#lastName", "");
                await browser.JsSendKeys("#address1", "");
                await browser.JsSendKeys("#city", "");
                await browser.JsSendKeys("#state", "");
                //string selectRandomState = $"let selectState = document.querySelector('#state').value='{Faker.Address.StateAbbr()}';";
                //await browser.ExecuteScript(selectRandomState);
                //await browser.JsSendKeys("#zipcode", "");
                await browser.JsSendKeys("#email", "");
                await browser.JsSendKeys("#telephone", "");
                await Task.Delay(2000);

                await browser.SendKeys("#firstName", Faker.Person.FirstName);
                //await Task.Delay(2000);
                await browser.SendKeys("#lastName", Faker.Person.LastName);
                //await Task.Delay(2000);
                await browser.SendKeys("#address1", Faker.Address.StreetAddress());

                await CorrectZip();

                //await Task.Delay(2000);
                //await browser.SendKeys("#city", Faker.Address.City());
                //await Task.Delay(2000);
                //await browser.JsSendKeys("#state", Faker.Address.StateAbbr());
                //string selectRandomState = @"let selectState = document.querySelector('#state');
                //let states = selectState.querySelectorAll('option');
                //var index = Math.floor(Math.random() * (states.length - 1)) + 1;
                //document.querySelector('label[for=state]').textContent = index;
                //selectState.selectedIndex = index; ";
                //await browser.ExecuteScript(selectRandomState, true);
                //await Task.Delay(2000);
                await browser.SendKeys("#email", Faker.Internet.Email());
                //await Task.Delay(2000);
                await browser.SendKeys("#telephone", Faker.Phone.PhoneNumber());


                await Task.Delay(2000);
                await browser.Click("#shipping-content > form > button");
                //string clickContinue = "document.querySelector('#shipping-content > form > button').click()";
            ...
            bool buttonVerifyAddress = await browser.ElementVisible("#shipping-content > form > div.LightBox__overlay.d-block > div > div > div.d-flex.flex-wrap > div:nth-child(4) > button", "ExceptionCheckoutVerifyAddres");
            if (buttonVerifyAddress)
            {
                return await VerifyAddress();
            }
            else
            {
                bool divOptionShipping = await browser.ElementVisible("#shipping-method > form > div.flex-wrap.d-flex > div:nth-child(2) > div > label > div", "ExceptionCheckout");
                if (divOptionShipping)
                {
                ...
                {
                    browser.Reload();
                    bool inputFirstName = await browser.ElementVisible("#firstName", "ExceptionCheckoutreload");
                    if (inputFirstName)
                    {
                        return await Checkout();
                    }
        ...
        private async Task<bool> VerifyVisibleState()
        {
            try
            {
                bool divState = await browser.ElementVisible("#state", "ExceptionNotFoundState", 10);
                if (!divState)
                {
                    browser.Reload();
                    bool inputFirstName = await browser.ElementVisible("#firstName", "ExceptionVerifyVisibleState");
                    if (inputFirstName)
                    {
                        return await Checkout();
                    }
        ...
        private async Task CorrectZip()
        {
            try
            {
                await browser.JsSendKeys("#zipcode", string.Empty);
                await Task.Delay(2000);
                await browser.SendKeys("#zipcode", Faker.Address.ZipCode());
            }
            catch (Exception) { }

            bool inputCity = await browser.ElementValue("#city", string.Empty, "ExceptionCorrectZip", 5);
            if (inputCity)
            {
                await CorrectZip();
            }
        ...
        private async Task<bool> VerifyAddress()
        {
            try
            {
               //browser.Screenshot("5.Checkout");
                await ConsoleProgressGeneral("Resolviendo teorema de Gibbs.", 30);
                //await Task.Delay(5000);
                await browser.Click("#shipping-content > form > div.LightBox__overlay.d-block > div > div > div.d-flex.flex-wrap > div:nth-child(3) > button");
                //string clickButtonUseThisAddress = "document.querySelector('#shipping-content > form > div.LightBox__overlay.d-block > div > div > div.d-flex.flex-wrap > div:nth-child(4) > button').click()";
                //await browser.ExecuteScript(clickButtonUseThisAddress);
                //await Task.Delay(2000);
                //await browser.Click("#shipping-content > form > button");
                //string clickContinue = "document.querySelector('#shipping-content > form > button').click()";
                //await browser.ExecuteScript(clickContinue);
            ...
            {
                browser.Reload();
                bool inputFirstName = await browser.ElementVisible("#firstName", "ExceptionVerifyAddressReload");
                if (inputFirstName)
                {
                    return await Checkout();
                }
        ...
        private async Task<bool> Shipping()
        {
            try
            {
               //browser.Screenshot("6.Checkout");
                await ConsoleProgressGeneral("Resolviendo teorema de Gibbs.", 30);
                // await browser.Click("#shipping-method > form > div.flex-wrap.d-flex > div:nth-child(2) > div > label > div");
                string clickRadioShipping = "document.querySelector('#shipRadio1').click()";
                await browser.ExecuteScript(clickRadioShipping);
                await Task.Delay(2000);
                //await browser.Click("#shipping-method > form > button");
                string clickContinue = "document.querySelector('#shipping-method > form > button').click()";
                await browser.ExecuteScript(clickContinue);

            }
            catch (Exception) { }
            //
            bool divOptionShipping = await browser.ElementVisible("#paymentFrameContain > iframe", "ExceptionShipping");
            if (divOptionShipping)
            {
               ...
               //browser.Screenshot("7.Last");
                ...
                if (CreditCards.Count > 0)
                {
                    ...
                    for (int i = 0; i < creditCardsCount;)
                    {
                        ...
                        bool latestData = await LatestData();
                        if (latestData)
                        {
        ...
        private async Task<bool> Payment(string number, string month, string year, string cvv)
        {
            ...
            bool divOptionShipping = await browser.ElementInvisible("#shipping-method > form > div.flex-wrap.d-flex > div:nth-child(2) > div > label > div", "ExceptionLogInWait");
            if (divOptionShipping)
            {
                return await Shipping();
            }
        ...
        private async Task<bool> LatestData()
        {
            try
            {
