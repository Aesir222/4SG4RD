...
namespace Asgard
{
    public partial class OdinBrwnlls : Form, IForm
    {
        ...
        private async Task<bool> StartOdin()
        {
            ...
            bool anchorClosePopup = await browser.ElementVisible("#ltkpopup-close-button > a", "1.ExceptionStartOdinClosePopup");
            if (anchorClosePopup)
            {
                await browser.ExecuteScript("document.querySelector('#ltkpopup-close-button > a').click()");
            }

            bool divItemStock = await browser.ElementVisible("#ctl00_ContentPlaceHolderColMain_divProdList > div:nth-child(2).media.listing", "1.ExceptionStartOdin");
            if (divItemStock)
            {
                return await Item();
            }
        ...
        private async Task<bool> Item()
        {
            try
            {
                //browser.Screenshot("2.StartOdin");
                ...
                string clickItemInStock = @"let allItems = document.querySelectorAll('#ctl00_ContentPlaceHolderColMain_divProdList > div > div > div.group2 > p.status > span:not(.outOfStock)');
                    let randItem = allItems[Math.floor(Math.random() * allItems.length)].parentNode.parentNode.parentNode.firstElementChild;
                    randItem.click();";
                await browser.ExecuteScript(clickItemInStock);
            }
            catch (Exception) { }

            bool buttonAddCart = await browser.ElementVisible("#btnAdd", "2.ExceptionItemAddCart");
            if (buttonAddCart)
            {
                return await RandomAddToCart();
            }
            else
            {
                bool anchorRounds = await browser.ElementVisible("#divBoxRounds > a:nth-child(1)", "2.1.ExceptionItemRoundsAddToCartAnchorRounds");
                if (anchorRounds)
                {
                    return await RoundsAddToCart();
                }
            }
        ...
        private async Task<bool> RandomAddToCart()
        {
            ...
            try
            {
                ...
                string clickRandomAddToCart = @"let elementsPrice = document.querySelectorAll('#priceContainer > span > p > span');
                    ...
                    for(let elementPrice of elementsPrice)
                    {
                        ...
                        if(price < 10)
                        {
	                        if(elementPrice.parentNode.parentNode.parentNode.parentNode.childNodes[5].getAttribute('class') == 'addToCart')
                            {
            ...
            bool anchorMasterCheckout = await browser.ElementVisible("#linkMasterCheckout", "3.ExceptionRandomAddToCart");
            if (anchorMasterCheckout)
            {
        ...
        private async Task<bool> RoundsAddToCart()
        {
            try
            {
                //browser.Screenshot("3.ItemAlt");
                await ConsoleProgressGeneral("Importando anclajes de desprogramación séctica.", 30);
                await browser.DisableAlerts();
                bool anchorRounds = await browser.ElementVisible("#divBoxRounds > a:nth-child(1)", "3.ExceptionItemAltRoundsAddToCartAnchorRounds");
                if (anchorRounds)
                {
                    ...
                    if (rounds)
                    {
                        bool buttonAddToCartAlt = await browser.ElementVisible("#ctl00_ContentPlaceHolderColMain_ucApperal_btnAdd", "3.1.ExceptionItemAltRoundsAddToCartButtonAddToCartAlt");
                        if (buttonAddToCartAlt)
                        {
                            ...
                            if (preAddtoCart)
                            {
                                bool inputAgeYes = await browser.ElementVisible("#btnAgeYes", "3.2.ExceptionItemAltRoundsAddToCartInputAgeYes");
                                if (inputAgeYes)
                                {
            ...
            bool anchorMasterCheckout = await browser.ElementVisible("#linkMasterCheckout", "3.ExceptionItemAlt");
            if (anchorMasterCheckout)
            {
                return await Checkout();
            }

            await ConsoleProgressGeneral("Importando anclajes de desprogramación séctica. ¡Fallo!", 30, "Fail");
            return false;
        ...
        //    try
        //    {
        //        ////browser.Screenshot("4.AddToCart");
        ...
        private async Task<bool> Checkout()
        {
            ...
            try
            {
                //browser.Screenshot("4.Checkout");
                await ConsoleProgressGeneral("Interrelacionando distribución regular de caos.", 40);
                await browser.DisableAlerts();
                string clickCheckout = @"document.querySelector('#linkMasterCheckout').click();";
                await browser.ExecuteScript(clickCheckout);
            }
            catch (Exception) { }

            bool inputEmail = await browser.ElementVisible("#txtEmail", "4.ExceptionCheckout");
            if (inputEmail)
            {
                return await Login();
            }
        ...
        private async Task<bool> Login()
        {
            ...
            bool inputFirstName = await browser.ElementVisible("#txtFirstName", "5.ExceptionLogin");
            if (inputFirstName)
            {
        ...
        private async Task<bool> Shipping()
        {
            try
            {
                //browser.Screenshot("6.Shipping");
                await ConsoleProgressGeneral("Incremento de las conductas laborales.", 60);
                await browser.DisableAlerts();
                await browser.JsSendKeys("#txtFirstName", Faker.Name.FirstName());
                await Task.Delay(500);
                await browser.JsSendKeys("#txtLastName", Faker.Name.LastName());
                await Task.Delay(500);
                await browser.JsSendKeys("#txtShippingAddr1", Faker.Address.StreetAddress());
                await Task.Delay(500);
                await browser.JsSendKeys("#txtCity", Faker.Address.City());
                await Task.Delay(500);
                await browser.JsSendKeys("#selState", Faker.Address.StateAbbr());
                await Task.Delay(500);
                await browser.JsSendKeys("#txtZip", Faker.Address.ZipCode("#####"));
                await Task.Delay(500);
                await browser.ExecuteScript("document.querySelector('#btnSaveAndContinue').click()");
            }
            catch (Exception) { }

            bool inputCreditCard = await browser.ElementVisible("#txtCreditCardNumber", "6.ExceptionShipping");
            if (inputCreditCard)
            {
                return await Last();
            }
            ...
            {
                bool buttonContinue = await browser.ElementVisible("#btnQASContinue", "6.1.ExceptionShippingCorrectAddress");
                if (buttonContinue)
                {
                    return await CorrectAddress();
                }
            }
        ...
        private async Task<bool> CorrectAddress()
        {
            try
            {
                //browser.Screenshot("6.5.CorrectAddress");
            ...
            try
            {
                ...
                if (CreditCards.Count > 0)
                {
                    ...
                    for (int i = 0; i < creditCardsCount;)
                    {
                        ...
                        bool latestData = await Payment(number, month, year, cvv);
                        if (latestData)
                        {
        ...
        private async Task ClickButtonOK()
        {
            ...
            try
            {
                //browser.Screenshot("9.ClickButtonOK");
                await browser.DisableAlerts();
        ...
        private async Task ClickPlaceOrder()
        {
            try
            {
                //browser.Screenshot("9.ClickPlaceOrder");
                await browser.Scroll(0, 500);
        ...
        private async Task<bool> Forseti()
        {
            ...
            try
            {
                //browser.Screenshot("10.Forseti");
                await ConsoleProgressDetail("Insuflando furia subatómica.", 90);
                await browser.DisableAlerts();
                bool hellheim = await browser.ElementVisible("div.fancybox-wrap.fancybox-desktop.fancybox-type-html.fancybox-opened > div > div > div > div > div > div > input", "10.ExceptionHelhaim", 30);
                if (hellheim)
                {
                    await ClickButtonOK();
                    return false;
                ...
                else
                {
                    ...
                    bool valhalla = await browser.ElementInnerTextContent("#ctl00_ContentPlaceHolderColMain_divConfirmNumber > h1", "Thank", "10.1.ExceptionValhalla");
                    if (valhalla)
                    {
                        return true;
                    }
                    ...
                    {
                        bool buttonOk = await browser.ElementInnerTextContent("#ctl00_bodytag > div.fancybox-wrap.fancybox-desktop.fancybox-type-html.fancybox-opened > div > div > div > div > div", "Unable", "ExceptionHelheim2");
                        if (buttonOk)
                        {
                            await EditPayment();
                            return false;
        ...
        private async Task EditPayment()
        {
            try
            {
                //browser.Screenshot("11.EditPayment");
